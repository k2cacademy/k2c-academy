...
export function CoachChat({ session, profile }: { session: string; profile: Profile }) {
  ...
  const send = async (text: string, voice = false) => {
    ...
    try {
      ...
      setMessages(m => [
        ...m,
        { id: `a-${Date.now()}`, role: "assistant", content: reply, created_at: new Date().toISOString() },
      ]);
    } catch {
      setMessages(m => [
        ...m,
        { id: `e-${Date.now()}`, role: "assistant", content: "I'm right here with you 💜. Try sending that again in a moment.", created_at: new Date().toISOString() },
      ]);
    } finally {
      setSending(false);
    }
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="mx-auto max-w-3xl w-full px-4 pt-4">
              <div className="rounded-2xl p-4 flex items-center gap-3" style={{ background: "rgba(20,20,30,0.6)", border: "1px solid rgba(147,51,234,0.3)", backdropFilter: "blur(12px)" }}>
                <div className="h-12 w-12 rounded-full flex items-center justify-center font-bold text-white" style={{ background: "linear-gradient(135deg,#5B21B6,#9333EA)", boxShadow: "0 0 20px rgba(147,51,234,0.6)" }}>
                  AI
                </div>
            ...
            <div className="relative border-t border-white/5 bg-black/40 backdrop-blur-xl">
              ...
              >
                ...
                <button
                  ...
                  style={{ background: "linear-gradient(135deg,#FACC15,#FBBF24)", boxShadow: "0 0 20px rgba(250,204,21,0.5)" }}
                  aria-label="Send"
                >
          ...
          <div className="flex flex-col items-center justify-center h-full p-6">
            <div className="w-full max-w-sm">
              <div className="text-center mb-8">
                <div className="h-24 w-24 rounded-full mx-auto flex items-center justify-center text-3xl font-bold mb-4" style={{ background: "linear-gradient(135deg,#5B21B6,#9333EA)", boxShadow: "0 0 40px rgba(147,51,234,0.5)" }}>
                  AI
                </div>
                ...
                <div className="mt-3">
                  {isInnerCircle ? (
                    <span className="px-3 py-1 rounded-full text-xs font-semibold bg-yellow-500/20 text-yellow-400 border border-yellow-500/30">
                      ⭐ Inner Circle — 10 mins/day
                    </span>
                  ) : (
                    <span className="px-3 py-1 rounded-full text-xs font-semibold bg-green-500/20 text-green-400 border border-green-500/30">
                      Free Trial — 3 mins/day
                    </span>
                  )}
                </div>
              </div>
              ...
              {minutes && (
                <div className="rounded-2xl p-4 mb-6 text-center" style={{ background: "rgba(20,20,30,0.6)", border: "1px solid rgba(147,51,234,0.3)" }}>
                  <p className="text-2xl font-bold text-white">
                    {minutes.free_remaining + minutes.purchased} <span className="text-sm font-normal text-white/60">mins left today</span>
                  </p>
                  {minutes.purchased > 0 && (
                    <p className="text-xs text-blue-400 mt-1">+{minutes.purchased} purchased mins</p>
                  )}
                </div>
              )}
              ...
              <button
                onClick={openCall}
                className="w-full py-4 rounded-2xl font-bold text-black text-lg flex items-center justify-center gap-2"
              ...
              <button
                ...
                className="w-full py-3 rounded-2xl font-semibold text-white/70 text-sm mt-3 border border-white/10 hover:border-white/20 transition"
              >
                ⚡ Top up minutes
              </button>
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/60">
                    {isInnerCircle ? "✨ Unlimited edits — Inner Circle" : "3 free edits — upgrade for unlimited"}
                  </p>
                </div>
              </div>
            ...
            <div className="mx-auto max-w-3xl w-full px-4 pt-6 pb-24 space-y-4">
              ...
              <div className="rounded-2xl p-4" style={{ background: "rgba(20,20,30,0.6)", border: "1px solid rgba(147,51,234,0.3)" }}>
                ...
                <div className="flex items-center justify-between py-2">
                  ...
                  <span className={`text-sm font-bold ${isInnerCircle ? "text-yellow-400" : "text-green-400"}`}>
                    {isInnerCircle ? "⭐ Inner Circle" : "Free Trial"}
                  </span>
                </div>
                ...
                {!isInnerCircle && (
                  <button
                    onClick={innerCircleWA}
                    className="w-full mt-3 py-3 rounded-xl font-bold text-black flex items-center justify-center gap-2"
              ...
              <div className="rounded-2xl p-4" style={{ background: "rgba(20,20,30,0.6)", border: "1px solid rgba(147,51,234,0.3)" }}>
                ...
                {[
                  ...
                  <button
                    key={item.label}
                    onClick={item.action}
                    className="w-full text-left py-3 border-b border-white/5 last:border-0 text-sm text-white/80 hover:text-white transition flex items-center justify-between"
                  >
                    {item.label}
        ...
        <div className="fixed inset-0 z-40 bg-black/70 backdrop-blur-sm flex items-center justify-center px-4">
          <div className="w-full max-w-md rounded-2xl p-6" style={{ background: "linear-gradient(135deg,#1a0a2e,#2d1b4e)", border: "1px solid rgba(147,51,234,0.5)" }}>
            ...
            <div className="mt-4 flex gap-2">
              <button onClick={() => setActionModal(null)} className="flex-1 py-2.5 rounded-xl bg-white/10 text-white text-sm hover:bg-white/15">
                Cancel
              </button>
              <button
                onClick={() => {
        ...
        <div className="fixed inset-0 z-40 bg-black/85 backdrop-blur-md flex items-center justify-center px-4">
          <div className="w-full max-w-md rounded-3xl p-7 text-center" style={{ background: "linear-gradient(135deg,#1a0a2e,#2d1b4e)", border: "1px solid rgba(250,204,21,0.4)", boxShadow: "0 0 60px rgba(250,204,21,0.25)" }}>
            ...
            <a
              ...
              style={{ background: "#FACC15", boxShadow: "0 0 40px rgba(250,204,21,0.5)" }}
            >
