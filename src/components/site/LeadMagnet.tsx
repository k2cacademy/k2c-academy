...
import { sendLeadMagnet } from "@/lib/lead-magnet.functions";

export function LeadMagnet() {
  const [submitting, setSubmitting] = useState(false);
  const [done, setDone] = useState(false);
  const sendFn = useServerFn(sendLeadMagnet);
  ...
  const onSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      setSubmitting(true);
      await sendFn({ data: { name, email, whatsapp } });
      setDone(true);
          ...
          <div className="grid gap-0 lg:grid-cols-2">
            ...
            <div className="relative bg-hero-glow p-8 sm:p-12">
              ...
              <p className="mt-4 text-sm leading-relaxed text-muted-foreground sm:text-base">
                A free, no-fluff PDF showing you the everyday skills you can
                monetize this week — straight from the K2Ç playbook.
              </p>
            </div>
