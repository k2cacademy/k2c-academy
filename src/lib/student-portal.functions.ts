...
// ---------- Book Editor (session-based) ----------
const BOOK_EDITOR_MODES: Record<string, { label: string; system: string }> = {
  "professional-polish": {
...
export const getBookEditorState = createServerFn({ method: "POST" })
  .inputValidator((i) => sessionInput.parse(i))
  .handler(async ({ data }) => {
    const userId = verifySession(data.session);
    const { data: profile } = await supabaseAdmin
      .from("student_profiles")
      .select("inner_circle_status, book_edits_used")
      .eq("user_id", userId)
      .maybeSingle();
    ...
    return {
      ...
      modes: Object.entries(BOOK_EDITOR_MODES).map(([id, m]) => ({ id, label: m.label })),
    };
  });

export const runBookEditor = createServerFn({ method: "POST" })
  .inputValidator((i) =>
    z
      .object({
  ...
  .handler(async ({ data }) => {
    const userId = verifySession(data.session);
    const mode = BOOK_EDITOR_MODES[data.mode];
    ...
    await supabaseAdmin
      .from("student_profiles")
      .update({ book_edits_used: newUsed })
      .eq("user_id", userId);
